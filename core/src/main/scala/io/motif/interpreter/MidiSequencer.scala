...
class MidiSequencer(midi: MidiWriter):
  ...
  def add(segment: ILSegment): Unit =
    given Context = Context(midi.ticksPerBeat, 4)
    val trackLength = next(segment, midi.ticksPerBeat)
    next(ILRest, trackLength)

  private def next(expr: ILExpr, ticks: Int)(using ctx: Context): Int =
    import ctx.*
    expr match
      ...
      case ILDuration(expr, power, dots) =>
        ...
        next(expr, ticks)(using ctx.copy(noteLength = length))
      case ILOctave(expr, value) =>
        next(expr, ticks)(using ctx.copy(octave = octave + value))
      case ILFragment(exprs) =>
        ...
        exprs.foldLeft(ticks): (tickSum, expr) =>
          next(expr, tickSum)(using fragmentCtx)
        ticks + noteLength
      case ILHarmony(exprs) =>
        exprs.map(next(_, ticks)).last
      case ILSegment(exprs) =>
        exprs.foldLeft(ticks):
          (tickSum, expr) => next(expr, tickSum)
      case ILKey(tonic, mode, expr) =>
        next(expr, ticks)(using ctx.copy(key = Key.ofMode(mode, tonic.symbol, tonic.accidental.getOrElse(0))))
      case ILTime(_, bpn, expr) =>
        val length = (noteLength * (noteValue / bpn.toDouble)).toInt
        next(expr, ticks)(using ctx.copy(noteLength = length, noteValue = bpn))
      ...
      case ILInstrument(program, expr) =>
        ...
        next(expr, ticks)(using instrCtx)
  ...
  private def unitLength(expr: ILExpr)(using Context): Double = expr match
    ...
    case ILInstrument(_, expr) => unitLength(expr)
